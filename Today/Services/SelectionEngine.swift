...
final class SelectionEngine {
    ...
    /// Reveals a task in the structured tree (expanding collapsed ancestors)
    /// and scrolls the tree to it, without touching the selection. Used by
    /// minimap clicks, which navigate the tree but don't pick a task.
    func revealAndScrollInStructured(_ task: TodayTask) {
        revealInStructured(task)
        requestScroll([(.structured, task.id)])
    }
